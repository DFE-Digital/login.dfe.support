...
describe('when updating the UserDevices active index in azure search', () => {
  let get;
  let set;
  ...
  beforeEach(() => {
    ...
    updateActiveIndex = require('./../../../src/infrastructure/userDevices/azureSearch').updateActiveIndex;
  });

  it('then it should update CurrentIndex_UserDevices key to index name in redis', async () => {
    await updateActiveIndex('new-index-name');

    expect(set.mock.calls).toHaveLength(1);
    expect(set.mock.calls[0][0]).toBe('CurrentIndex_UserDevices');
