...
describe('when getting a users organisations mapping from api', () => {
  ...
  it('then it should call associated-with-user resource with user id', async () => {
    await getOrganisationById(organisationId, correlationId);

    expect(fetchApi.mock.calls).toHaveLength(1);
    expect(fetchApi.mock.calls[0][0]).toBe('http://organisations.test/organisations/org-1');
  ...
  it('should return null on a 401 or 404 response', async () => {
    ...
    let result = await getOrganisationById(organisationId, correlationId);
    ...
    result = await getOrganisationById(organisationId, correlationId);
    expect(result).toEqual(null);
  });
  ...
  it('should return false on a 409 response', async () => {
    ...
    const result = await getOrganisationById(organisationId, correlationId);
