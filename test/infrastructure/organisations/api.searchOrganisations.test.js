...
describe('when getting a page of organisations from api', () => {
  ...
  it('then it should call organisations resource with page number and filterStatus if provided', async () => {
    ...
    await searchOrganisations(criteria, filterByCategories, filterByStatusFilled, pageNumber, correlationId);

    expect(fetchApi.mock.calls).toHaveLength(1);
    expect(fetchApi.mock.calls[0][0]).toBe('http://organisations.test/organisations?search=criteria&page=1&filterstatus=123&filterstatus=145');
    expect(fetchApi.mock.calls[0][1]).toMatchObject({
      method: 'GET',
    });
  });
  ...
  it('then it should call organisations resource with page number and filterCategories if provided', async () => {
    ...
    await searchOrganisations(criteria, filterByCategoriesFilled, filterByStatus, pageNumber, correlationId);

    expect(fetchApi.mock.calls).toHaveLength(1);
    expect(fetchApi.mock.calls[0][0]).toBe('http://organisations.test/organisations?search=criteria&page=1&filtercategory=33&filtercategory=66');
    expect(fetchApi.mock.calls[0][1]).toMatchObject({
      method: 'GET',
    });
  });
  ...
  it('then it should call organisations resource with page number, and both filterStatus and filterCategories, if provided', async () => {
    const filterByStatusFilled = ['123', '145'];
    const filterByCategoriesFilled = ['33', '66'];
    await searchOrganisations(criteria, filterByCategoriesFilled, filterByStatusFilled, pageNumber, correlationId);

    expect(fetchApi.mock.calls).toHaveLength(1);
    expect(fetchApi.mock.calls[0][0]).toBe('http://organisations.test/organisations?search=criteria&page=1&filtercategory=33&filtercategory=66&filterstatus=123&filterstatus=145');
    expect(fetchApi.mock.calls[0][1]).toMatchObject({
      method: 'GET',
    });
  });
