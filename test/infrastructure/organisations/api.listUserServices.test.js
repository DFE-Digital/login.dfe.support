...
const { listUserServices } = require('../../../src/infrastructure/organisations/api');
...
describe('when getting a users organisations mapping from api', () => {
  ...
  it('then it should call associated-with-user resource with user id', async () => {
    await listUserServices(page, pageSize, correlationId);

    expect(fetchApi.mock.calls).toHaveLength(1);
    expect(fetchApi.mock.calls[0][0]).toBe('http://organisations.test//services/associated-with-user?page=1&pageSize=10');
  ...
  it('should return null on a 401 response', async () => {
    ...
    const result = await listUserServices(page, pageSize, correlationId);
  ...
  it('should raise an exception on any failure status code that is not 401, 404 or 409', async () => {
    ...
    const act = () => listUserServices(page, pageSize, correlationId);
