...
const { setUserAccessToOrganisation } = require('../../../src/infrastructure/organisations/api');
...
describe('when getting a users organisations mapping from api', () => {
  ...
  it('then it should call associated-with-user resource with user id', async () => {
    await setUserAccessToOrganisation(userId, organisationId, roleId, correlationId, status, reason);
  ...
  it('should return null on a 404 response', async () => {
    ...
    const result = await setUserAccessToOrganisation(userId, organisationId, roleId, correlationId, status, reason);
  ...
  it('should raise an exception on any failure status code that is not 401, 404 or 409', async () => {
    ...
    const act = () => setUserAccessToOrganisation(userId, organisationId, roleId, correlationId, status, reason);
