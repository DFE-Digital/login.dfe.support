...
describe("when getting a users services mapping from api", () => {
  ...
  it("then it should call users resource with user id", async () => {
    await getServiceIdentifierDetails(
      serviceId,
      identifierKey,
      identifierValue,
      correlationId,
    ...
    expect(fetchApi.mock.calls[0][0]).toBe(
      "http://access.test/services/service-1/users?filteridkey=identifier-key-1&filteridvalue=idenfier-value-1",
    );
  ...
  it("should return false on a 404 response", async () => {
    ...
    const result = await getServiceIdentifierDetails(
      serviceId,
      identifierKey,
      identifierValue,
      correlationId,
