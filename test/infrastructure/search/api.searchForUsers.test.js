...
describe("when searching for users", () => {
  ...
  it("then it should call associated-with-user resource with user id", async () => {
    ...
    expect(fetchApi.mock.calls[0][0]).toBe(
      "http://search.test/users?criteria=testCriteria&page=1",
    );
  ...
  it.each([
    ...
    "should return a url with %s as a sortBy param when %s is provided",
    async (expectedResult, testSortBy) => {
      ...
      expect(fetchApi.mock.calls).toHaveLength(1);
      expect(fetchApi.mock.calls[0][0]).toBe(
        `http://search.test/users?criteria=testCriteria&page=1&sortBy=${expectedResult}`,
      );
      expect(fetchApi.mock.calls[0][1]).toMatchObject({
        method: "GET",
      });
  ...
  it("should throw an exception if the sortBy value is not recognised", async () => {
    ...
    const act = () =>
      searchForUsers(criteria, pageNumber, testSortBy, sortDirection, filters);

    await expect(act).rejects.toThrow(
      expect.objectContaining({
        name: "Error",
        message:
          "Error searching for users with criteria testCriteria (page: 1) - Unexpected user sort field willFail",
      }),
    );
  });
  ...
  it("should include sortDirection in the url if one is specified", async () => {
    ...
    expect(fetchApi.mock.calls).toHaveLength(1);
    expect(fetchApi.mock.calls[0][0]).toBe(
      "http://search.test/users?criteria=testCriteria&page=1&sortDirection=asc",
    );
    expect(fetchApi.mock.calls[0][1]).toMatchObject({
      method: "GET",
    });
  ...
  it("it should include a filter if one is provided", async () => {
    ...
    expect(fetchApi.mock.calls).toHaveLength(1);
    expect(fetchApi.mock.calls[0][0]).toBe(
      "http://search.test/users?criteria=testCriteria&page=1&filter_organisationTypes=52",
    );
    expect(fetchApi.mock.calls[0][1]).toMatchObject({
      method: "GET",
    });
  ...
  it("should raise an exception when a 400 is returned", async () => {
    ...
    const act = () =>
      searchForUsers(criteria, pageNumber, sortBy, sortDirection, filters);

    await expect(act).rejects.toThrow(
      expect.objectContaining({
        name: "Error",
        message:
          "Error searching for users with criteria testCriteria (page: 1) - Cannot read properties of undefined (reading 'numberOfPages')",
      }),
    );
  });
  ...
  it("should raise an exception with an Error message on any failure status code that is not 400, 403, 404", async () => {
    ...
    await expect(act).rejects.toThrow(
      expect.objectContaining({
        name: "Error",
        message:
          "Error searching for users with criteria testCriteria (page: 1) - Server Error",
      }),
    );
  });
});
