...
describe("When deleting unused indexes from Azure Search", () => {
  ...
  beforeAll(() => {
    ...
    jest.doMock("ioredis", () =>
      jest.fn().mockImplementation(() => {
        ...
        redisMock.set(
          "CurrentIndex_Users",
          "users-58457890-ba74-49ae-86eb-b4a144649805",
        );
        redisMock.set(
          "UnusedIndexes_Users",
    ...
    deleteUnusedIndexes =
      require("./../../../src/infrastructure/users/azureSearch").deleteUnusedIndexes;
  });
  ...
  it("then it should delete users in indexes marked as unused that are still not used", async () => {
    ...
    expect(fetchApi.mock.calls[0][0]).toBe(
      "https://test-search.search.windows.net/indexes/users-4771d85e-f3ef-4e71-82ca-30f0663b10c9?api-version=2016-09-01",
    );
  ...
  it("then it should not delete users in indexes marked as unused that are now used", async () => {
    ...
    fetchApi.mock.calls.forEach((call) => {
      expect(call[0]).not.toMatch(
        /indexes\/users-58457890-ba74-49ae-86eb-b4a144649805/,
      );
    });
  });
  ...
  it("then it should mark any indexes that are not current as unused", async () => {
    ...
    expect(actual).toBe(
      JSON.stringify(["users-4771d85e-f3ef-4e71-82ca-30f0663b10c9"]),
    );
  });
});
