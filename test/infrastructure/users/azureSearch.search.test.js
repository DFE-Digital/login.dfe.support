...
describe('when searching for a user in azure search', () => {
  ...
  it('then it should search the current index for the criteria and page, with a page size of 25 and ordered by name if no order specified', async () => {
    ...
    expect(fetchApi.mock.calls[0][0]).toBe('https://test-search.search.windows.net/indexes/test-index/docs?api-version=2016-09-01&search=test&$count=true&$skip=0&$top=25&$orderby=name');
  ...
  it('then it should search the current index for the criteria and page, with a page size of 25 and ordered by specified field if possible', async () => {
    ...
    expect(fetchApi.mock.calls).toHaveLength(1);
    expect(fetchApi.mock.calls[0][0]).toBe('https://test-search.search.windows.net/indexes/test-index/docs?api-version=2016-09-01&search=test&$count=true&$skip=0&$top=25&$orderby=email desc');
    expect(fetchApi.mock.calls[0][1]).toMatchObject({
      method: 'GET'
    });
  });
  ...
  it('then the search value is set to lowercase and white space is removed', async () => {
    ...
    expect(fetchApi.mock.calls[0][0]).toBe('https://test-search.search.windows.net/indexes/test-index/docs?api-version=2016-09-01&search=testuser&$count=true&$skip=0&$top=25&$orderby=name');
    expect(fetchApi.mock.calls[0][1]).toMatchObject({
      method: 'GET'
    });
  });
