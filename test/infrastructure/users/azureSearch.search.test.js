...
describe("when searching for a user in azure search", () => {
  ...
  beforeEach(() => {
    ...
    fetchApi.mockImplementation(() => {
      return {
        "@odata.context":
          "https://sbsa-search.search.windows.net/indexes('users-7170516e-5671-4ea7-8e52-adfc901c73c3')/$metadata#docs",
        "@odata.count": 49,
        value: [
          {
  ...
  it("then it should search the current index for the criteria and page, with a page size of 25 and ordered by specified field if possible", async () => {
    ...
    expect(fetchApi.mock.calls).toHaveLength(1);
    expect(fetchApi.mock.calls[0][0]).toBe(
      "https://test-search.search.windows.net/indexes/test-index/docs?api-version=2016-09-01&search=test&$count=true&$skip=0&$top=25&$orderby=email desc",
    );
    expect(fetchApi.mock.calls[0][1]).toMatchObject({
      method: "GET",
    });
