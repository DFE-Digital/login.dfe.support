...
const updateIndex = async (accessRequests, index) => {
  ...
  const accessRequestMap = accessRequests.map((accessRequest) => {
    ...
    if (accessRequest.organisation.category === establishment) {
      ...
      accessRequest.organisation.category === multiAcademyTrust ||
      accessRequest.organisation.category === singleAcademyTrust
    ) {
    ...
    return {
      ...
      nameSearch: accessRequest.name
        ? accessRequest.name.replace(/\s/g, "").toLowerCase()
        : "",
      email: accessRequest.email,
      emailSearch: accessRequest.email
        ? accessRequest.email.replace("@", "").toLowerCase()
        : "",
...
const deleteUnusedIndexes = async () => {
  ...
  const indexesAppearingUnused = indexesResponse.value
    ...
    .filter(
      (x) =>
        x !== currentIndexName &&
        x.toLowerCase().indexOf("accessrequests-") !== -1,
    );
  await client.set(
