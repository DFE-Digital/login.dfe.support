...
const loadUsers = async (newIndexName, correlationId) => {
  ...
  while (hasMorePages) {
    ...
    const pageOfUsers = await directories.getPageOfUsers(pageNumber, 250, false, correlationId);
...
const getAllUsers = async (correlationId) => {
  ...
  while (hasMorePages) {
    ...
    const page = await directories.getPageOfUsers(pageNumber, 250, false, correlationId);
