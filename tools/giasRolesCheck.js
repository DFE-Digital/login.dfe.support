...
const getNumberOfAvailableRoles = async () => {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (policyResult.length === service.requiredRolesForService) {
      ...
      console.log(
        `Pushing to ${service.requiredRolesForService} available roles : `,
        i,
      );
      ...
      console.log(
        `Pushing to more than ${service.requiredRolesForService} available roles`,
      );
    } else {
      ...
      console.log(
        `Pushing to less than ${service.requiredRolesForService} available roles`,
      );
    }
  }
  console.log(
    "Equal to required roles length : " + equalToRequiredRolesAvailable.length,
  );
  console.log(
    "More than required roles available length " +
      moreThanRequiredRolesAvailable.length,
  );
  console.log(
    "Less than required roles available length " +
      lessThanRequiredRolesAvailable.length,
  );
...
const updateUserRoles = async () => {
  ...
  if (policy.equalToRequiredRolesAvailable.length > 0) {
    for (let i = 0; i < policy.equalToRequiredRolesAvailable.length; i++) {
      ...
      let somthing = await serviceNotificationsClient.notifyUserUpdated({
  ...
  if (policy.moreThanRequiredRolesAvailable.length > 0) {
    const stream = createWriteStream(
      `${homeDirectory}/Desktop/DSI/${service.name}-more-than.csv`,
    );
    ...
    for (let i = 0; i < policy.moreThanRequiredRolesAvailable.length; i++) {
      ...
      stream.write(
        `=HYPERLINK(https://support.signin.education.gov.uk/users/${currentPolicy.userId}/organisations/${currentPolicy.organisationId}/services/${currentPolicy.serviceId})`,
      );
      stream.write("\r\n");
    }
  }
  ...
  if (policy.lessThanRequiredRolesAvailable.length > 0) {
    const stream = createWriteStream(
      `${homeDirectory}/Desktop/DSI/${service.name}-less-than.csv`,
    );
  ...
  console.log(
    `Updated roles for ${policy.equalToRequiredRolesAvailable.length} users`,
  );
  console.log(
    `Removed ${policy.noRolesAvailable.length} users from service as they had no available roles`,
  );
};
